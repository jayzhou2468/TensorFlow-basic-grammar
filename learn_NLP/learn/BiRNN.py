...
class BiRNN:
    def __init__(self, embedding_size, rnn_size, layer_size, vocab_size, attn_size, sequence_length,
        ...
        with tf.name_scope('bi_rnn'), tf.variable_scope('bi_rnn'):
            self.outputs, _, _ = tf.contrib.rnn.static_bidirectional_rnn(lstm_fw_cell_m, lstm_bw_cell_m, inputs,
                                                                    dtype=tf.float32)
        print(tf.shape(self.outputs))
        # 此时outputs的维度为shape=(batch_size, sequence_length, 2 * hidden_size)
        # 这里的outputs默认是将正反方向的结果拼接到一起　
        # define attention
        with tf.name_scope('attention'), tf.variable_scope('attention'):
            attention_w = tf.Variable(tf.truncated_normal([2*rnn_size, attn_size], stddev=0.1),
                                      name='attention_w')
            ...
            for t in range(sequence_length):
                ...
                u_t = tf.tanh(tf.matmul(self.outputs[t], attention_w) + attention_b)
                u_list.append(u_t)
            ...
            self.final_output = tf.reduce_sum(self.outputs * alpha_trans, 0)    # 加权输出向量累加
